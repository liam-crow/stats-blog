...
I've been co-admin of a little page on Facebook that caters to a niche audience of AFL statistics nerds known as [Useless AFL Stats](https://www.facebook.com/uselessaflstats), where Aaron and I find out stats that have no relevance to anything at all, and will never be useful to anyone, ever. And that got me thinking, why should I have all the fun finding these nuggets of gold?
...
Once you've downloaded both, open up your newly installed Rstudio. It should automatically find R and open up an interface. Think of R as the frame, steering wheel and engine of a car. You can get pretty far with just that, but Rstudio completes the car. God that's a terrible analogy.
...
Let's save these results, we may want to use it later. In R we use an assignment command `<-`, which looks like a backwards arrow. You can assign a result to any string of characters.
...
If you look at the 'Environment' tab in the top right section, we can see our newly created variables, which we can use later on. Variables can be used in commands with each other too.
