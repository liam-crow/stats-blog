...
If you have any questions during this tutorial, you can tweet at me [\@crow_data_sci](https://twitter.com/crow_data_sci){target="blank"}. If you've got a useless stat, feel free to tweet at us [\@UselessStatsAFL](https://twitter.com/UselessStatsAFL){target="blank"} or message our facebook page.

Ok, let get into the setup!

# R and RStudio Installation

The program that we'll be using to generate stats is [R](https://www.r-project.org/){target="blank"}, a powerful tool commonly used by professional statisticians and data scientists in academia and industry, but we'll be using it to shitpost about AFL stats.

We'll be downloading R from here [https://cloud.r-project.org/](https://cloud.r-project.org/){target="blank"} and choose the version for your OS. I'd recommend all the default file locations and setup. If you are using a MAC OS, click `R-4.2.x.pkg` which is the executable.

We'll also be using RStudio, a Graphical User Interface (GUI) that allows for easier use of the R language. Download the free version here at [https://rstudio.com/products/rstudio/download/](https://rstudio.com/products/rstudio/download/){target="blank"}.
...
Writing commands in the script and pressing enter will not execute the command, but will take you to a newline. To execute a line, use `Ctrl` + `Enter`. Press `Ctrl` + `S`to save your script, and you should see it appear in the 'Files' tab on the right of RStudio.
...
Ignore the warning about "Rtools", it's a windows development tool that we won't be using.
...
Now that we have everything set up, we can dive right in to the stats. We are going to load in data from [afltables.com](https://afltables.com/afl/afl_index.html){target="blank"} using [fitzRoy](https://github.com/jimmyday12/fitzRoy){target="blank"}, an R package put together by [James Day](https://twitter.com/jamesday87){target="blank"} that contains most of the match data in a consistent structure.
