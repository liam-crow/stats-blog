---
title: Can you tell if a players' game is worthy of a brownlow vote?
author: Liam Crowhurst
date: '2020-07-07'
slug: can-you-tell-if-a-players-game-is-worthy-of-a-brownlow-vote
categories:
  - AFL Analysis
tags:
  - R
...
The Brownlow medal, one of the most prestigious AFL awards, is notoriously difficult to predict. You can read this article from [Fat Stats](https://www.thefatstats.com/post/predicting-the-brownlow-is-hard) for a great look as to why. Here's a quick summary:
...
> Only 3 out of 44 playes can actually recieve a vote!

6% of players per game recieve a vote! Further more, 1 out of 44 recieve the 3 votes. Thats is a ridiculously small sample size, and predicting rare events is one of the trickiest things to do in predictive modelling. I'm gonna try anyway.
...
Once a low dimensional represenation has been calculated and visualised, we expect to see different structures between 0 vote games and vote worthy games. Here are the two methods I'll be using.
...
PCA reduces the dimensions of the dataset while preserving as much of the total variabilty as possible. PCA outputs a linear combination of vectors, where each principal component explains a proportion of variability in the dataset. [1](https://royalsocietypublishing.org/doi/10.1098/rsta.2015.0202)

UMAP is a non-linear dimension reduction technique and uses a fuzzy topological structure to model the manifold. The end result is a low dimensional projection of the data that is closest to its equivalent fuzzy topological structure. [2](https://umap-learn.readthedocs.io/en/latest/)

Using each method, we can compare the visualisations and make judgements about which better represents the structure of the data.
...
Dimensionality reduction is the process of taking these 100's of dimensions and leaving us with 2 (or 3) that most accurately correspond to its higher dimensional shape.
# The first attempts
### Principal component analysis (no scaling)

For the next sections I'll be using the 2018 Home and Away Season, and sourcing the data from [fitzRoy](https://github.com/jimmyday12/fitzRoy). I'll be using `prcomp` from `stats` to generate the principal components.
...
Let have a look at our non-linear dimensionality reduction technique, using the same data as before and visualise two components.
...
One important thing to note is that the intial attempts treated all the players equally, regardless of name, playing condition, home advantage: purely based on performance. For the next attempts I'll be scaling the statistics by game, which will take out the inherent game bias. A player that has the most touches in a wet game at 25 will no longer be treated the same as a dry 35 disposal game. 

Each game will grouped and scaled, where each stat willl have its mean adjusted to 0 and the variance to 1. When hovering over the points, keep in mind the following definitions:

- stat = 0: an average amount for that game
- stat >0 0: higher than average
- stat <0 0: lower than average
...
Next post on this topic will be the generation of predictions using lower dimensional embeddings, and compare their effectiveness against the PCA, and against multiple components. Given this exporation was on 2018 data, I'll generate predicitons for 2019 and compare the results.
...
- There is (most likely) no discernable difference between 1, 2 and 3 vote games
-
